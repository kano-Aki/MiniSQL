...
bool TablePage::UpdateTuple(const Row &new_row, Row *old_row, Schema *schema, Transaction *txn,
                            LockManager *lock_manager, LogManager *log_manager,bool& isNotEnough) {
  ...
  memmove(GetData() + free_space_pointer + tuple_size - serialized_size, GetData() + free_space_pointer,
          tuple_offset - free_space_pointer);////Move all the tuples in front of old forward, leaving space for new
  ////new_row and old_row have the same slot_num
...
bool TablePage::GetNextTupleRid(const RowId &cur_rid, RowId *next_rid) {
  page_id_t cur_page=cur_rid.GetPageId();
  page_id_t page_id=GetTablePageId();
