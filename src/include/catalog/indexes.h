...
class IndexMetadata {
 ...
 private:
  ...
  ////store the column index which index is on it
};
 ...
 */
  void Init(IndexMetadata *meta_data, TableInfo *table_info, BufferPoolManager *buffer_pool_manager) {
    ...
    // Step3: call CreateIndex to create the index
      ...
      std::vector<uint32_t> key_map=meta_data_->GetKeyMapping();
      std::vector<Column *> cols;
      for (auto it:key_map) cols.push_back(new Column(table_info->GetSchema()->GetColumn(it)));
      key_schema_=new Schema(cols, true);
